...
# virtual environment created by .cursor/Dockerfile, then pre-caches the
# pre-commit hook environments so the first lint run is fast.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${REPO_ROOT}"

# The install phase does not necessarily inherit the image's PATH, so activate
# the shared virtual environment explicitly and use absolute interpreter paths.
VENV_DIR="/opt/venv"
...
if [ -f .pre-commit-config.yaml ]; then
    "${VENV_DIR}/bin/pre-commit" install-hooks
fi
